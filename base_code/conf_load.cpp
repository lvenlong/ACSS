...
/*bool ConfLoader::load_conf(const std::string& conf_file, ConfUnit& conf_unit)
{
    ...
    if (!conf_unit.is_group()) {
        std::cerr << "error: load_conf: The 3rd argument need to be (LiLong::ConfUnit::UT_GROUP)" << std::endl;
        return false;
    }
    ...
    } catch (LiLong::ConfigurationException& e) {
...
ConfUnit* ConfLoader::load_conf(const char* conf_filename)
{
    ...
    if (NULL != root) {
        ...
        while (conf_file.good()) {
            ...
            if (LiLong::ConfLoader::is_white_line(line) || LiLong::ConfLoader::is_comment_line(line)) {
                continue;
            }

            if (0 != LiLong::ConfLoader::handle_line(line, root, current)) {
                delete root;
                root = NULL;
                break;
            }
...
/*void ConfLoader::parse(const std::string& conf_filename, ConfUnit& conf_unit)
{
    ...
    if (!conf_file) {
        throw LiLong::ConfigurationException("failed to open conf file: " + conf_filename);
    }
...
{
    if (conf_file.fail()) {
        throw LiLong::ConfigurationException("failed to parse conf file: " + _filename);
    }
...
bool ConfLoader::is_white_line(const char* line)
{
    if (NULL == line) {
        ...
        return LiLong::StringUtil::trim(line).empty();
    }
}
...
bool ConfLoader::is_comment_line(const char* line)
{
    if (NULL == line) {
        ...
        std::string line_trimed = LiLong::StringUtil::ltrim(line);
...
{
    paths.clear();
    std::string line_trimed = LiLong::StringUtil::trim(line);
    ...
    LiLong::StringUtil::split_string(line_trimed, ".", v, false);
    if (0 == v.size()) {
        return -1;
    }
    ...
    for (itr = v.begin(); itr != v.end(); ++itr) {
        std::string s = LiLong::StringUtil::trim(*itr);
        if (s.empty()) {
            return -1;
        } else {
            paths.push_back(s);
...
int32_t ConfLoader::handle_as_item(const char* line, std::string& key, std::string& value)
{
    ...
    if (std::string::npos == pos) {
        ...
        key = LiLong::StringUtil::trim(line_trimed.substr(0, pos));
        if (key.empty()) {
            return -1;
        }
        if (pos != line_trimed.size() - 1) {
            value = LiLong::StringUtil::trim(line_trimed.substr(pos + 1));
        } else {
            value = "";
        }
...
int32_t ConfLoader::push_item(ConfUnit* current, const std::string& key, const std::string& value)
{
    ...
    ConfUnit* tmp = (*current)[LiLong::StringUtil::trim(key)];
    if (NULL != tmp) {
        return -1;
    }
    current->insert(LiLong::StringUtil::trim(key), new(std::nothrow) ConfUnit(LiLong::StringUtil::trim(value)));
    return 0;
}
}
