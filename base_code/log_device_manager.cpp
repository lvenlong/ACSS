...
namespace LiLong
{
    void load_default_device()
    {
        ...
        ILogDevice* default_device = new(std::nothrow) FileLogDevice("./LiLong.log", "LiLong", LOGLEVEL_WARN, LiLong_LOG_SPLIT_POLIT_BY_SIZE, 500*1024*1024, -1, "%D [%N] %T {%F(%L)} %C");
        if(NULL == default_device)
        {
            std::cerr << "Create default device 'LiLong' failed." << std::endl;
            exit(0);
        }
        ...
        if(0 != ret)
        {
            std::cerr << "Open default device failed: " << LiLong_strerror_r(ret, buffer, BUFSIZ) << std::endl;
        ...
        {
            std::cerr << "Add device 'LiLong' failed." <<std::endl;
            delete default_device;
            exit(0);
        }
    }

    static int32_t get_loglevel_from_logdev_confunit(LiLong::ConfUnit& logdev)
    {
    ...
    static int32_t get_split_policy_from_logdev_confunit(LiLong::ConfUnit& logdev)
    {
        std::string split_policy_valstr = (*logdev["split_policy"]).to_string();
        int32_t split_policy_value;

        if("SIZE" == split_policy_valstr)
            split_policy_value = LiLong_LOG_SPLIT_POLIT_BY_SIZE;
        else if("TIME" == split_policy_valstr)
            split_policy_value = LiLong_LOG_SPLIT_POLIT_BY_TIME;
        else
        {
    ...
    static int64_t get_maxsize_from_logdev_confunit(LiLong::ConfUnit& logdev, int32_t split_policy)
    {
        int64_t maxsize_value = 0;
        if(split_policy == LiLong_LOG_SPLIT_POLIT_BY_SIZE)
        {
    ...
    static int32_t get_life_circle_from_logdev_confunit(LiLong::ConfUnit& logdev, int32_t split_policy)
    {
        int32_t life_circle_value = 0;

        if(split_policy == LiLong_LOG_SPLIT_POLIT_BY_TIME)
    ...
    void load_conf_device(const char* conf_filepath)
    {
        ...
        LiLong::ConfUnit *root;
        //if(!LiLong::ConfLoader::load_conf(conf_filepath, root))
        if((root = LiLong::ConfLoader::load_conf(conf_filepath)) == NULL)
        {
        ...
        try
        {
            LiLong::ConfUnit& logdevs = *((*root)["log_devices"]);
            ...
            LiLong::ConfUnit::const_iterator citr;
            int32_t i;
            for(citr = logdevs.begin(), i = 1; citr != logdevs.end(); ++citr, ++i)
            {
                LiLong::ConfUnit& logdev = **citr;
                ...
                LiLong::ConfUnit* tmp_unit = logdev["file_use_pid"];
                int32_t file_use_pid = 0;
                if( tmp_unit != NULL  /*(*tmp_unit).get_type() != LiLong::ConfUnit::UT_NULL*/)
                {
    ...
    void log_open(const char* conf_filepath)
    {
        ...
        if(false == manager->is_device_exist("LiLong"))
        {
            load_default_device();
        }
    }
