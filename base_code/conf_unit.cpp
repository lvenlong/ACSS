...
int64_t ConfUnit::to_integer() const
{
    if (UT_INTEGER == get_type()) {
        ...
        std::string trimed_text = LiLong::StringUtil::trim(to_string());
        int64_t val;

        if (LiLong::StringUtil::str2int64(trimed_text.c_str(), val)) {
            return val;
        } else {
            std::stringstream ss;
            ss << "failed to cast string `" << _string << "' to integer";
            throw LiLong::ConfigurationException(ss.str());
        }
    } else {
        throw LiLong::ConfigurationException("failed to cast");
    }
}
