...
namespace LiLong
{
    ...
    AbstractLogDevice::AbstractLogDevice(const char* name, int32_t loglevel, int32_t split_policy, off_t max_size, int32_t max_record_interval, const char* log_format) : _loglevel(loglevel), _split_policy(split_policy), _max_size(max_size), _max_record_interval(max_record_interval)
    {
    ...
    FileLogDevice::FileLogDevice(
                                 ...
                                 const char* log_format) :
                                     AbstractLogDevice(name, loglevel, split_policy, max_size, max_record_interval, log_format), _fp(NULL)
                                     {
    ...
    int32_t FileLogDevice::open(void* param)
    {
        ...
        if(false == is_opened()){
            if(NULL != param){
                strncpy(_file_path, (char*)param, MAX_FILE_PATH_LEN);
            }
    ...
    int32_t FileLogDevice::write(const struct log_message_t& log_message)
    {
...

        {
            ...
            if(LiLong_LOG_SPLIT_POLIT_BY_TIME == split_policy){
